var Switcher = {
  ...
  autoHide: function() {
    if (Switcher.bar.hasClass('hidden')) return;

    var autoHideTimeout = setTimeout(Switcher.hideBar, 3000);
    Switcher.bar.hover(
      function() { clearTimeout(autoHideTimeout) },
      Switcher.autoHide
    );
  },
  ...
  showBar: function(speed) {
    ...
    Switcher.showButton.animate({"margin-top": "-8px", "margin-top": "-100px"}, speed, function() {
      ...
      Switcher.bar.removeClass('hidden');
    });
  },
